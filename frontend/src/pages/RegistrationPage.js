...
const RegistrationPage = ({ onRegister }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Basic validation
    const isStudentValid = role === 'student' && programme;
    const isFacultyValid = role === 'faculty' && department;

    if (name && email && password && (isStudentValid || isFacultyValid)) {
      // Pass the complete user object to the handler in App.js
      onRegister({ name, email, password, role, studentId, programme, department });
    } else {
      alert('Please fill out all required fields for your role.');
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          {role === 'faculty' && (
            <div className="input-group">
              ...
              <input type="text" id="department" value={department} onChange={(e) => setDepartment(e.target.value)} placeholder="e.g., Computer Science" required={role === 'faculty'} />
            </div>
          )}
